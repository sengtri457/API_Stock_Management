...
exports.getProductByCode = async (req, res) => {
    try {
        const product = await Product.findOne({product_code: req.params.code.toUpperCase()});
...
exports.createProduct = async (req, res) => {
    try {
        ...
        const existingProduct = await Product.findOne({product_code: product_code.toUpperCase()});
        ...
        res.status(404).json({success: false, message: "Error creating product", error: error.message});
    }
};
...
exports.updateProductStock = async (req, res) => {
    try {
        ...
        if (!["IN", "OUT", "ADJUSTMENT"].includes(type)) {
            return res.status(400).json({success: false, message: "Invalid transaction type. Must be IN, OUT, or ADJUSTMENT"});
        }
...
exports.getLowStockProducts = async (req, res) => {
    try {
        ...
        const lowStockProducts = products.filter((product) => product.quantity_in_stock<= product.reorder_level, );

    res.status(200).json({
      success: true, count: lowStockProducts.length, data: lowStockProducts, });
  } catch (error) {
    res.status(500).json({
      success: false, message: "Error fetching low stock products", error: error.message, });
  }
};
...
exports.searchProducts = async (req, res) => {
        ...
        // @access  Public
            ...
            try {
                const products = await Product.find({"category.category_id": req.params.categoryId});
        ...
        // @access  Public
            ...
            try {
                const products = await Product.find({"supplier.supplier_id": req.params.supplierId});

                res.status(200).json({success: true, count: products.length, data: products});
            } catch (error) {
                res.status(500).json({success: false, message: "Error fetching products by supplier", error: error.message});
            }
        };
        ...
        // @access  Public
            ...
            try {
                ...
                const stats = {
                    ...
                    low_stock_products: products.filter((product) => product.quantity_in_stock<= product.reorder_level, ).length, out_of_stock_products: products.filter(
        (product) => product.quantity_in_stock === 0,).length
                };

                res.status(200).json({success: true, data: stats});
            } catch (error) {
                res.status(500).json({success: false, message: "Error fetching inventory statistics", error: error.message});
            }
        };
